...
test('local is the default and needs nothing from the cloud', () => {
  assert.deepStrictEqual(chooseConnection({ mode: undefined, account: null, deviceId: '' }), { mode: 'local' });
  assert.deepStrictEqual(chooseConnection({ mode: 'local', account, deviceId: DEVICE }), { mode: 'local' });
});

...
// Narwal's sign-in tells the server the user is 14 or older, so the user
// confirms it first; only an explicit true counts.
function requireAdult(adult) {
  if (adult !== true) throw new NarwalCloudError('Confirm that you are 14 or older to sign in.', 'BAD_INPUT');
}
...
class CloudAccountManager extends EventEmitter {
  ...
  async requestEmailCode({ email, country, adult }) {
    const cleanMail = cleanEmail(email);
    const account = this._newAccount(country);
    requireAdult(adult);
    await account.requestEmailCode(cleanMail);
  }
  ...
  logout() {
    const account = this._account;
    // Best effort: signing out here never waits on or fails because of the
    // network; the local tokens are gone either way.
    if (account) account.logout().catch(() => {});
    this._account = null;

...
function uuidFromToken(token) {
  return tokenClaims(token).uuid || null;
}
...
class NarwalCloudAccount {
  ...
  static fromJSON(state = {}, options = {}) {
    ...
    account.method = state.method || null;
    return account;
  }
  ...
  /**
   * How this session looks, for comparing it with a working one. Shapes and
   * yes/no answers only: no ids, tokens or host names.
   */
  diagnostics() {
    const claims = tokenClaims(this._access);
    const tokenUuid = claims.uuid ? String(claims.uuid) : '';
    const uuid = this.uuid ? String(this.uuid) : '';
    let broker = null;
    if (this._broker) {
      try {
        const url = new URL(this._broker);
        broker = { scheme: url.protocol.replace(':', ''), port: url.port ? Number(url.port) : null };
      } catch (_) {
        broker = { scheme: 'invalid', port: null };
      }
    }
    return {
      signInMethod: this.method || 'unknown',
      uuidMatchesToken: Boolean(uuid) && uuid === tokenUuid,
      uuidLength: uuid.length,
      uuidHasDashes: uuid.includes('-'),
      tokenUuidLength: tokenUuid.length,
      tokenClaimKeys: Object.keys(claims).sort(),
      tokenIssuedMinutesAgo: claims.iat ? Math.round((Date.now() / 1000 - claims.iat) / 60) : null,
      broker,
    };
  }

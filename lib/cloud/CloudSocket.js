...
const { parseFrame, buildFrame } = require('../NarwalBinaryProtocol');
...
// Requests are sent one at a time: the next waits for this one's reply, or
// this long. A burst of requests loses replies over the cloud.
const CLOUD_REPLY_PACING_MS = 5000;
// The broadcasts the robot is asked to publish (active_robot_publish) plus
// firmware updates. Kept short on purpose: the broker confirms any number of
// subscriptions but appears to route only about the first ten of a session,
// and every request needs its own reply topic too.
const CLOUD_BROADCAST_TOPICS = [
...
class CloudSocket extends EventEmitter {
  constructor({
    ...
    this._replyWaiters = new Map(); // response topic -> resolve
  ...
  async _start() {
    ...
    // An MQTT 5 broker explains why it drops a session in its DISCONNECT.
    client.on('disconnect', (packet) => {
      this._stats.disconnectReason = packet && packet.reasonCode !== undefined ? packet.reasonCode : null;
    });
  ...
  _onConnect() {
    const topics = CLOUD_BROADCAST_TOPICS.map((topic) => `${this._base}/${topic}`);
  ...
  _onMessage(topic, payload) {
    this._stats.rawReceived += 1;
    if (!isDeviceTopic(topic, this._productId, this._deviceId)) {
      const shape = this._maskTopic(topic);
      this._stats.otherTopics = [...this._stats.otherTopics.filter((t) => t !== shape), shape].slice(-5);
      return;
    }
    ...
    this._stats.byTopic[short] = (this._stats.byTopic[short] || 0) + 1;
    const waiter = this._replyWaiters.get(topic);
    if (waiter) waiter();
  ...
  async _publish(topic, body) {
    ...
    this._client.publish(topic, buildCloudPayload(this._account.uuid, body), {
      ...
      // Not awaited (see above), but a refusal is worth knowing about.
      if (!err) return;
      this._stats.publishErrors += 1;
      this._stats.lastPublishError = err.code !== undefined ? `${err.code} ${err.message}` : err.message;
    });
    await this._waitForReply(responseTopic);
  }

  // Resolves when the reply arrives on responseTopic, or after the pacing
  // timeout; never rejects, so the queue keeps moving.
  _waitForReply(responseTopic) {
    return new Promise((resolve) => {
      const done = () => {
        clearTimeout(timer);
        if (this._replyWaiters.get(responseTopic) === done) this._replyWaiters.delete(responseTopic);
        resolve();
      };
      const timer = setTimeout(done, CLOUD_REPLY_PACING_MS);
      this._replyWaiters.set(responseTopic, done);
    });
  }

  // A topic with the product and device IDs replaced, safe to show.
  _maskTopic(topic) {
    return String(topic).split(this._productId).join('<product>').split(this._deviceId)
      .join('<device>')
  ...
  stats() {
    return { ...this._stats, lastTopics: [...this._stats.lastTopics], byTopic: { ...this._stats.byTopic } };
  }
...
module.exports = {
  CloudSocket, CONNECTING, OPEN, CLOSED, SUBSCRIBE_TIMEOUT_MS, CLOUD_REPLY_PACING_MS,
};

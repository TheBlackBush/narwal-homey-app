...
function chooseConnection({
  ...
  if (!account || !account.signedIn) {
    return { mode: 'cloud', error: 'Sign in to your Narwal account in the app settings, or switch Connection to Local.' };
  }
  if (!deviceId) {
    return { mode: 'cloud', error: 'This robot has no device ID yet. Remove it and add it again, then switch Connection to Narwal cloud.' };
  }

...
class NarwalBinaryProtocol {
  ...
  normalizeBaseStatus(decoded = {}) {
    ...
    const offDock = dockField11 === 1 || dockField47 === 2
      || Number((field3 && field3['3']) || 0) === 2 || dockSubState === 2;
    if (offDock && state === C.RobotState.DOCKED) {
      state = workingStatus === WORKING_STATE.DOCKED_V2 ? C.RobotState.CLEANING : C.RobotState.UNKNOWN;
    }
    // Task completed (19) can persist after the robot is back on the dock.
    if (workingStatus === WORKING_STATE.TASK_COMPLETED && !returning && !offDock
      && (dockSubState === 1 || dockField11 >= 2 || dockField47 === 1 || dockField47 === 3)) {
      state = C.RobotState.DOCKED;
    }
    const docked = !offDock && (state === C.RobotState.DOCKED || (
    ...
    const charging = docked ? (battery === null || battery < 100) : false;
    ...
    return {
      ...
      currentRoomId: docked ? null : undefined,
    };
  }

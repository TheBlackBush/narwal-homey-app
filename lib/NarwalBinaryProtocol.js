...
// ActiveRobotPublish.Request as the official app sends it on connect and then
// every publishDuration / 2: {1: TopicDuration{1 working status, 2 display
// map, 3 base status} broadcast intervals in ms, 2: publish window in ms}.
// Over the cloud a robot publishes only while this window is open.
function buildActivePublishRequest({ topicIntervalMs = 2000, publishWindowMs = 60000 } = {}) {
  return encodeMessage([
    encodeBytesField(1, encodeMessage([
      encodeVarintField(1, topicIntervalMs),
      encodeVarintField(2, topicIntervalMs),
      encodeVarintField(3, topicIntervalMs),
    ])),
    encodeVarintField(2, publishWindowMs),
  ]);
}
...
function decodeProto(buf, depth = 0) {
  ...
  while (offset < buf.length) {
    ...
    if (wire === 0) {
      ...
      offset = len.offset;
      if (offset + len.value > buf.length) throw new Error(`truncated field ${Math.floor(key.value / 8)}`);
...
class NarwalBinaryProtocol {
  ...
  // AppEvent 1 = app start, 2 = device page entered (what the official app
  // sends when the robot's screen opens). `legacy` adds the older requests
  // local robots are known to accept.
  buildWakeFrames({ legacy = true } = {}) {
    return [
      buildFrame(this.fullTopic('common/notify_app_event'), encodeVarintField(1, 1)),
      buildFrame(this.fullTopic('common/notify_app_event'), encodeVarintField(1, 2)),
      ...this.buildSubscriptionFrames({ legacy }),
  ...
  // The official app's sequence when its robot screen opens: keep-publishing,
  // device page entered, base status. Used over the cloud, where extra
  // requests appear to cost replies.
  buildCloudWakeFrames() {
    return [
      buildFrame(this.fullTopic('common/active_robot_publish'), buildActivePublishRequest()),
      buildFrame(this.fullTopic('common/notify_app_event'), encodeVarintField(1, 2)),
      buildFrame(this.fullTopic('status/get_device_base_status'), Buffer.alloc(0)),
    ];
  }

  buildSubscriptionFrames({ legacy = true } = {}) {
    const frames = [buildFrame(this.fullTopic('common/active_robot_publish'), buildActivePublishRequest())];
    if (legacy) {
      frames.push(
  ...
  normalizeBaseStatus(decoded = {}) {
    ...
    const dockPresence = Number((field3 && field3['3']) || 0);
    // Each firmware marks "off the dock" in its own field, so any of these
    // wins over a docked status code. Newer Flow 2 firmware (v01.09.10.02)
    // reports working status 2 while cleaning away from the dock; older
    // firmware used 2 for docked.
    const offDock = dockField11 === 1 || dockField47 === 2 || dockPresence === 2 || dockSubState === 2;
    const onDock = !offDock && (dockPresence === 1 || dockPresence === 6 || dockSubState === 1
      || dockActivity > 0 || dockField11 >= 2 || dockField47 === 1 || dockField47 === 3);
    if (offDock && state === C.RobotState.DOCKED) {
      state = workingStatus === WORKING_STATE.DOCKED_V2 ? C.RobotState.CLEANING : C.RobotState.IDLE;
    }
    // Task completed (19) can persist after the robot is back on the dock.
    if (workingStatus === WORKING_STATE.TASK_COMPLETED && !returning && onDock) {
      state = C.RobotState.DOCKED;
    }
    // Some firmware sends partial messages (only battery, or only dock
    // fields). Dock fields alone still place the robot; with no dock fields
    // and no status code the dock state is unknown (null) and kept.
    if (workingStatus === null && onDock) state = C.RobotState.DOCKED;
    let docked;
    if (offDock) docked = false;
    else if (state === C.RobotState.DOCKED || (state !== C.RobotState.CLEANING && onDock)) docked = true;
    else if (workingStatus === null || workingStatus === WORKING_STATE.TASK_COMPLETED) docked = null;
    else docked = false;
    // Off the dock the robot cannot be charging.
    let charging = null;
    if (docked === true) charging = battery === null || battery < 100;
    else if (docked === false) charging = false;
    ...
    return {
      ...
      // Room comes from working_status; a docked robot has no current room.
      currentRoomId: docked === true ? null : undefined,
    };
  }

  normalizeWorkingStatus(decoded = {}) {
    // Field 2 is covered area. Field 13 is a dock bag drying timer, not area.
    const areaRaw = decoded['2'] === undefined ? null : toFloat32(decoded['2']);
    ...
    // Only covered area or elapsed time show a clean in progress; a room id
    // alone is also sent after the robot has docked.
    const active = (area || 0) > 0 || (time || 0) > 0;
    return {
      raw: decoded,
      rawMode: null,

...
const { cloudPairEntries } = require('./cloud/cloudPairing');

const DISCOVERY_PROBE_TIMEOUT_MS = 8000;
// The pairing view gives up after 20 s, so the manual check must finish first.
const MANUAL_PROBE_TIMEOUT_MS = 12000;
...
class NarwalHomeyDriver extends Homey.Driver {
  ...
  // Robots announced over mDNS, listed at once without contacting them.
  // Discovery must never block pairing, so any failure returns an empty list.
  async _listFoundRobots() {
    return markAdded(this._discoveredRobots(), this._existingIds());
  }

  _discoveredRobots() {
    try {
      return uniqueResults(Object.values(this.homey.discovery.getStrategy('narwal').getDiscoveryResults() || {}));
    } catch (err) {
  ...
  _existingIds() {
    return this.getDevices()
      .flatMap((device) => [String(device.getData().id), device.getStoreValue('deviceId')])
      .filter(Boolean);
  }

  // What the pairing screen shows: the local search or the account's robots.
  _pairingMode() {
    const cloud = this.homey.app && this.homey.app.cloud;
    if (!cloud) return { mode: 'local', signedIn: false };
    const { mode, signedIn } = cloud.status();
    return { mode, signedIn };
  }

  async _listCloudRobots() {
    const cloud = this.homey.app && this.homey.app.cloud;
    const account = cloud ? cloud.getAccount() : null;
    if (!account || !account.signedIn) throw new Error('Sign in to your Narwal account in the app settings first.');
    const robots = await account.listRobots();
    return cloudPairEntries(robots, this.model, this._existingIds(), this._discoveredRobots());
  }

  // Builds the device for a robot chosen from the account list; the local IP
  // is filled in when mDNS has seen the robot.
  async _cloudDevice({ deviceId } = {}) {
    const entry = (await this._listCloudRobots()).find((robot) => robot.deviceId === deviceId);
    if (!entry) throw new Error('Robot not found on your Narwal account.');
    if (entry.group !== 'match') throw new Error(`This is a ${entry.modelName}. Add it under that model instead.`);
    const device = this._createPairDevice({
      ip: entry.ip,
      port: DEFAULT_PORT,
      status: { deviceId: entry.deviceId, topicPrefix: `/${entry.productId}` },
    });
    device.name = entry.name || device.name;
    return device;
  }

  // Checks one found robot: its model, and a ready-to-add device when it is
  // this driver's model. A silent robot is reported, not thrown.
  async _identifyRobot({ ip, suffix = null } = {}) {
    if (!IPV4_RE.test(String(ip || ''))) throw new Error('Not an IPv4 address.');
    let probe = null;
    let error = null;
    try {
      probe = await NarwalClient.probe({ ip, port: DEFAULT_PORT, timeoutMs: DISCOVERY_PROBE_TIMEOUT_MS });
    } catch (err) {
      error = err;
    }
    ...
    entry.responded = !error;
    if (entry.group === 'match') {
  ...
  async onPair(session) {
    ...
    session.setHandler('validate', async (input = {}) => {
      ...
      // Mock mode is for development only and never reachable from pairing.
      const useMock = process.env.NARWAL_MOCK === '1' && Boolean(input.dev_mock);
      const { model } = this;
    ...
    session.setHandler('pairing_mode', async () => this._pairingMode());
    session.setHandler('cloud_robots', async () => ({ robots: await this._listCloudRobots() }));
    session.setHandler('cloud_add', async (data) => ({ device: await this._cloudDevice(data) }));
    session.setHandler('discover', async () => ({ robots: await this._listFoundRobots() }));
    session.setHandler('identify', async (robot) => this._identifyRobot(robot));

...
const { uniqueResults, buildPairEntry } = require('./Discovery');
...
class NarwalHomeyDriver extends Homey.Driver {
  ...
  async _discoverRobots() {
    ...
    const existingIds = new Set(this.getDevices().map((device) => String(device.getData().id)));
    const found = uniqueResults(results);
    const probed = await Promise.all(found.map(({ suffix, ip }) => NarwalClient.probe({
      ip, port: DEFAULT_PORT, timeoutMs: DISCOVERY_PROBE_TIMEOUT_MS,
    }).then((probe) => ({ suffix, ip, probe }), (error) => ({ suffix, ip, error }))));
    return probed.map(({
      suffix, ip, probe, error,
    }) => {

  ...
  return (Array.isArray(rooms) ? rooms : [])
    .map((room, index) => {
      ...
      return {
        id,
        name: name || fallback,
        subtype: room && room.subtype !== undefined ? Number(room.subtype) : 0,
        category: room && room.category !== undefined ? Number(room.category) : 0,
        instanceIndex: room && room.instanceIndex !== undefined ? Number(room.instanceIndex) : 0,
      };
    })
    .filter((room) => room.id);
}
...
class NarwalDevice extends Homey.Device {
  ...
  async onSettings({ changedKeys, newSettings }) {
    ...
    if (changedKeys.some((k) => relevant.includes(k))) {
      ...
      this._stopClient();
      // Defer so the new settings are persisted before we read them back.
      this.homey.setTimeout(() => this._startClient(), 500);
    }
  }
  ...
  async _onStatus(status) {
    ...
    await this._fireStatusTriggers(status);
    this._prev = {
      state: status.state,
  ...
  async _fireStatusTriggers(status) {
  ...
  async _onMap(map) {
    await this._saveRooms(map.rooms || []);
  ...
  async _renderMap(map) {
    ...
    if (!this._mapImage) {
      this._mapImage = await this.homey.images.createImage();
      this._mapImage.setStream(async (stream) => {
        stream.write(Buffer.from(this._mapSvg || '', 'utf8'));
        stream.end();
      });
      await this.setCameraImage('map', 'Map', this._mapImage);
    } else {
      await this._mapImage.update();
    }
  }
  ...
  async startCleaning() {
    ...
    await this._requireClient().startClean();
  }
  ...
  async setFanSpeed(fanSpeed) {
    ...
    await this._requireClient().setFanSpeed(fanSpeed);
    await this.setCapabilityValue('narwal_fan_speed', fanSpeed).catch(() => {});
  }
  ...
  async refreshRoomsAndMap() {
    ...
    if (map) {
      await this._onMap(map);
    } else {
      await this._saveRooms(rooms);
    }
    return rooms;
  }

  async cleanRooms(roomIds, { source = 'clean_rooms' } = {}) {
    const ids = parseRoomIds(roomIds);
    if (!ids.length) throw new Error('No rooms selected.');
    this._validateRoomIds(ids);
    await this._recordCommand('clean_rooms', { source, roomIds: ids });
    await this._requireClient().cleanRoom(ids);
  }

...
class NarwalDevice extends Homey.Device {
  ...
  _settings() {
    ...
    return {
      ...
      connectionMode: s.connection_mode || 'local',
    };
  }
  ...
  _startClient() {
    ...
    const connection = chooseConnection({
      mode: cfg.connectionMode, account: cloudAccount, deviceId: cfg.deviceId, mock: cfg.mock,
    });
    if (connection.error) {
  ...
  async onSettings({ changedKeys, newSettings }) {
    ...
    const relevant = ['ip', 'port', 'poll_interval', 'dev_mock', 'connection_mode'];
  ...
  // The user signed in or out of the Narwal account in the app settings.
  onCloudAccountChanged() {
    if (this._settings().connectionMode === 'cloud') this._restartClient();
  }

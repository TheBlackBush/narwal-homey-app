...
const { packStaticMap } = require('./NarwalMapCodec');
const { LiveTrail } = require('./LiveTrail');

// Live position pushes to the widget are sent at most this often.
const LIVE_PUSH_MS = 2000;
...
// Device page button order: Start, Pause, Resume, Stop, Dock, Locate.
const BUTTON_ORDER = ['button.start', 'button.pause', 'button.resume', 'button.stop', 'button.dock', 'button.locate'];
...
class NarwalDevice extends Homey.Device {
  ...
  _startClient() {
    ...
    this._client = new NarwalClient({
      ...
      savedMap: this.getStoreValue('static_map'),
    });
    ...
    this._client.on('live', (live) => this._onLive(live));
    // Kept so cleaning and the widget work before (or without) a fresh map,
    // for example in Cloud mode.
    this._client.on('staticMap', (staticMap) => {
      this.setStoreValue('static_map', packStaticMap(staticMap)).catch((e) => this.log(`map save skipped: ${e.message}`));
    });
  ...
  _stopClient() {
    if (this._livePushTimer) {
      this.homey.clearTimeout(this._livePushTimer);
      this._livePushTimer = null;
    }
  ...
  async _ensureRuntimeCapabilities() {
    ...
    await this._ensureButtonOrder();
  }

  // Homey shows buttons in the order the device holds them, and devices paired
  // before the order changed keep the old one. Capabilities cannot be moved,
  // so re-add the buttons (they carry no state) in the manifest order.
  async _ensureButtonOrder() {
    const current = this.getCapabilities().filter((id) => BUTTON_ORDER.includes(id));
    const wanted = BUTTON_ORDER.filter((id) => current.includes(id));
    if (current.join() === wanted.join()) return;
    this.log('Reordering buttons:', wanted.join(', '));
  ...
  async _onStatus(status) {
    ...
    // A new clean starts a new path; the last one stays visible until then.
    if (status.state === RobotState.CLEANING && prev && prev.state
      && prev.state !== RobotState.CLEANING && prev.state !== RobotState.PAUSED) {
      this._liveTrail().reset();
      this._pushedLiveSeq = 0;
    }
    this._prev = {
      state: status.state,
  ...
  _liveTrail() {
    if (!this._trail) this._trail = new LiveTrail();
    return this._trail;
  }

  // The robot's live position and path, from its display_map broadcasts. The
  // trail keeps raw map coordinates, converted when sent, so it stays right
  // when the map is re-rendered.
  _onLive(live) {
    const staticMap = this._client && this._client.lastStaticMap;
    if (!staticMap || !staticMap.border || !live) return;
    this._liveTrail().add((live.trail || []).map((point) => ({ x: point.x, y: point.y })));
    this._live = { robot: live.lostPosition ? null : live.robot, lostPosition: Boolean(live.lostPosition), at: Date.now() };
    const wait = this._lastLivePush === undefined ? 0 : LIVE_PUSH_MS - (Date.now() - this._lastLivePush);
    if (wait <= 0) this._pushLive();
    else if (!this._livePushTimer) {
      this._livePushTimer = this.homey.setTimeout(() => {
        this._livePushTimer = null;
        this._pushLive();
      }, wait);
    }
  ...
  _toDisplay(point) {
    const staticMap = this._client && this._client.lastStaticMap;
    const meta = this._mapRenderData && this._mapRenderData.meta;
    return MapParser.liveToDisplay(point, staticMap && staticMap.border, meta);
  }

  _displayPoints(points) {
    return points.map((point) => this._toDisplay(point)).filter(Boolean).map(({ x, y }) => ({ x, y }));
  }

  _pushLive() {
    if (!this._live || !this.homey.api || typeof this.homey.api.realtime !== 'function') return;
    this._lastLivePush = Date.now();
    const trail = this._liveTrail();
    const delta = trail.since(this._pushedLiveSeq);
    this._pushedLiveSeq = trail.length;
    try {
      this.homey.api.realtime('narwal:live', {
        deviceId: this.getId(),
        robot: this._toDisplay(this._live.robot),
        lostPosition: this._live.lostPosition,
        from: delta.from,
        points: this._displayPoints(delta.points),
        total: trail.length,
      });
    } catch (err) {
      this.log(`live push skipped: ${err.message}`);
    }
  }

  /** The live position and whole path, for the widget. */
  getLiveData() {
    const trail = this._liveTrail();
    return {
      robot: this._live ? this._toDisplay(this._live.robot) : null,
      lostPosition: Boolean(this._live && this._live.lostPosition),
      at: this._live ? this._live.at : null,
      points: this._displayPoints(trail.points),
      total: trail.length,
    };
  }

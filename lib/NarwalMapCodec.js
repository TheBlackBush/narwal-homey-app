...
/**
 * A static map in a compact, JSON-safe form for the device store: the grid
 * as run-length pairs [value, count, value, count, ...]. Everything else is
 * kept as decoded.
 */
function packStaticMap(staticMap) {
  if (!staticMap || !Array.isArray(staticMap.grid)) return null;
  const runs = [];
  const { grid } = staticMap;
  for (let i = 0; i < grid.length;) {
    let j = i + 1;
    while (j < grid.length && grid[j] === grid[i]) j += 1;
    runs.push(grid[i], j - i);
    i = j;
  }
  return {
    ...staticMap, grid: undefined, version: PACK_VERSION, runs,
  };
}

function unpackStaticMap(packed) {
  if (!packed || packed.version !== PACK_VERSION || !Array.isArray(packed.runs)) return null;
  const grid = [];
  for (let i = 0; i < packed.runs.length; i += 2) {
    for (let n = 0; n < packed.runs[i + 1]; n += 1) grid.push(packed.runs[i]);
  }
  const { version, runs, ...rest } = packed;
  return { ...rest, grid };
}

...
class NarwalClient extends EventEmitter {
  ...
  setFanSpeed(fanSpeed) {
    if (this.binaryMode) {
      ...
      if (level === undefined) return Promise.reject(new Error(`Unknown fan speed: ${fanSpeed}`));
      return this._sendBinaryCommand(C.BinaryCommandTopic.SET_FAN_LEVEL, encodeVarintField(1, level));
    }

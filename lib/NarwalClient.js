...
class NarwalClient extends EventEmitter {
  ...
  async _command(shortTopic, payload = Buffer.alloc(0), timeoutMs = C.COMMAND_TIMEOUT_MS, confirm = null) {
    ...
    const done = { ok: true, resultCode: C.CommandResult.SUCCESS, topic: shortTopic };
    const reply = this._sendBinaryCommand(shortTopic, payload, C.CLOUD_REPLY_TIMEOUT_MS)
      .then((response) => ({ response }), (error) => ({ error }));
    // The status often shows the result before the slow reply arrives.
    const shown = confirm ? this._waitForStatusMatch(confirm, C.CLOUD_CONFIRM_TIMEOUT_MS) : null;
    const first = await (shown ? Promise.race([reply, shown.then((matched) => (matched ? { shown: true } : reply))]) : reply);
    if (first.shown) return { ...done, confirmedBy: 'status' };
    if (first.response) return first.response;
    if (!/timed out/i.test(first.error.message)) throw first.error;
    if (!confirm) return { ...done, confirmedBy: 'sent' };
    if (await shown) return { ...done, confirmedBy: 'status' };
  ...
  async refreshMap({ force = false } = {}) {
    if (this.binaryMode) {
      ...
      await this._sendBinaryCommand(
        ...
        Buffer.alloc(0),
        this.cloud ? C.CLOUD_MAP_TIMEOUT_MS : C.SLOW_COMMAND_TIMEOUT_MS,
        (candidate, msg) => {

...
class NarwalClient extends EventEmitter {
  ...
  _startPolling() {
    ...
    this._pollTimer = setInterval(() => {
      ...
      if (this._connected && idle >= this.pollIntervalMs) {
        this.refreshStatus().catch((err) => this._log(`poll failed: ${err.message}`));
      }
    }, this.pollIntervalMs);
  }
    ...
    if (this.binaryMode) {
      ...
      if (map && map.meta && map.meta.mapId && ids.length) {
        return this._sendBinaryCommand(
          ...
          buildStartCleanPayload(ids, map.meta.mapId),
          C.SLOW_COMMAND_TIMEOUT_MS,
        );
      }
      return this._sendBinaryCommand(
        C.BinaryCommandTopic.START_PLAN,
        buildCleanPayload(),
        C.COMMAND_TIMEOUT_MS,
      );
    }
  ...
  setFanSpeed(fanSpeed) {
    if (this.binaryMode) {
      const level = C.FAN_SPEED_VALUES.indexOf(fanSpeed);
      if (level < 0) return Promise.reject(new Error(`Unknown fan speed: ${fanSpeed}`));
      return this._sendBinaryCommand(C.BinaryCommandTopic.SET_FAN_LEVEL, Buffer.from([0x08, level & 0x7f]));
    }
    return this._send((id) => this.protocol.buildSetFanSpeed(fanSpeed, id));
  }

  async cleanRoom(roomIds) {
    if (this.binaryMode) {
      const map = await this._getCleanableMap();
      ...
      return this._sendBinaryCommand(
        C.BinaryCommandTopic.START_CLEAN,
        buildStartCleanPayload(roomIds, map.meta.mapId),
        C.SLOW_COMMAND_TIMEOUT_MS,
      );
    }

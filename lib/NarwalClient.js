...
class NarwalClient extends EventEmitter {
  ...
  // A lost reply must not leave the widget without a map: retry while none
  // is loaded, backing off.
  _scheduleMapRetry() {
    if (this.lastStaticMap || this._closing || this._mapRetryTimer) return;
    ...
    this._mapRetryTimer = setTimeout(() => {
      ...
      if (this._connected && !this.lastStaticMap) this._requestMap('retry');
    }, delay);
  }
  ...
  _sendBinaryDiscovery() {
    // Discovery probes other product keys on the LAN; the cloud already
    // knows the robot.
  ...
  _sendBinaryWakeBurst() {
    this._sendBinaryFrames(this.cloud
      ? this.binaryProtocol.buildCloudWakeFrames()
      : this.binaryProtocol.buildWakeFrames());
  }

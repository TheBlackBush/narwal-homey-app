...
class NarwalClient extends EventEmitter {
  ...
  async start_() {
    if (this.binaryMode) {
      ...
      return this._sendBinaryCommand(
        ...
        buildStartCleanPayload(ids, map.meta.mapId, this._cleanOptions()),
        C.SLOW_COMMAND_TIMEOUT_MS,
      );
    }
  ...
  _cleanOptions() {
    const fan = FAN_LEVEL[this.fanSpeed];
    return fan === undefined ? {} : { fan };
  }
  ...
  async cleanRoom(roomIds) {
    if (this.binaryMode) {
      ...
      return this._sendBinaryCommand(
        C.BinaryCommandTopic.START_CLEAN,
        buildStartCleanPayload(roomIds, map.meta.mapId, this._cleanOptions()),
        C.SLOW_COMMAND_TIMEOUT_MS,
      );
    }

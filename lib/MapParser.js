...
const NARWAL_MODEL_ROOM_TYPE_OVERRIDES = {
  QxMSPG6VSO: {
    1: 'Master Bedroom',
    5: 'Bathroom',
    10: 'Corridor',
...
function narwalRoomName(room, id, productKey = '') {
  ...
  const overrides = NARWAL_MODEL_ROOM_TYPE_OVERRIDES[productKey] || {};
  const base = overrides[subtype] || NARWAL_ROOM_TYPE_NAMES[subtype] || 'Room';
...
const COLOR_UNASSIGNED_OBSTACLE = [80, 80, 80, 255];
const COLOR_FALLBACK = [180, 180, 180, 255];
const COLOR_TRANSPARENT = [0, 0, 0, 0];

function darkenColor(color) {
  return [Math.round(color[0] * 0.62), Math.round(color[1] * 0.62), Math.round(color[2] * 0.62), 255];
}

function roomColor(roomId, isWall) {
  const base = NARWAL_ROOM_COLORS[roomId - 1] || COLOR_FALLBACK;
  const color = isWall ? darkenColor(base) : base;
...
class MapParser {
  ...
  static parseNarwalMapResponse(decoded, now = 0, productKey = '') {
    ...
    const rooms = asArray(roomsField)
      ...
      .map((room, index) => {
        ...
        return {
          id,
          name: narwalRoomName(room, id, productKey),
  ...
  static toRenderData(map, options = {}) {
    ...
    for (let i = 0; i < pixels.length; i += 1) {
      ...
      if (value === 0x20) {
        ...
        color = COLOR_UNASSIGNED_OBSTACLE;
      } else if (value !== 0) {
        const roomId = value >> 8;
        const pixelType = value & 0xff;
        const wall = Boolean(pixelType & 0x10);
        color = roomColor(roomId, wall);
        if (!wall && roomId > 0) {
    ...
    const roomLabels = Object.values(roomStats)
      ...
      .map((stat) => ({
        ...
        x: Math.round(stat.x / stat.count),
        y: Math.round(stat.y / stat.count),
      }));
  ...
  static toSVG(map, options = {}) {
    ...
    map.rooms.forEach((room, i) => {
      ...
      const fill = room.color || palette[i % palette.length];
...
module.exports = {
  ...
  NARWAL_MODEL_ROOM_TYPE_OVERRIDES,
};

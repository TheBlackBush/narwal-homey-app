...
const ROOM_CODE_BASE = 10;
// Six room colour sets, as the Narwal app cycles; the widget holds the colours.
const ROOM_COLOR_SETS = 6;
...
class MapParser {
  ...
  /**
   * Widget render data (version 2): the cropped map as cell classes in
   * run-length pairs [code, count, ...], top row first, plus a room table.
   * Codes: 0 outside, 1 wall, 2 floor without a room, ROOM_CODE_BASE + i the
   * i-th room. The widget colours them, so it can follow the Homey theme.
   * Rooms carry a colour set index (neighbours always differ, starting from
   * the dock's room, as the Narwal app does), a label point inside the room
   * and a bounding box. The dock and robot are in the same display cells.
   */
  static toRenderData(map, options = {}) {
    ...
    // Display coordinates: x = column, y = height - 1 - raw row.
    const classOf = (value) => classifyCell(value);
    let minX = width;
    let minY = height;
    let maxX = -1;
    let maxY = -1;
    ...
    const pad = Number.isFinite(options.cropPadding) ? Math.max(0, options.cropPadding) : 2;
    const crop = {
      x: Math.max(0, minX - pad), y: Math.max(0, minY - pad), maxX: Math.min(width - 1, maxX + pad), maxY: Math.min(height - 1, maxY + pad),
    };
    const w = crop.maxX - crop.x + 1;
    const h = crop.maxY - crop.y + 1;

    // Rooms in map order, plus any room id seen only in the grid.
    const rooms = [];
    const byId = new Map();
    const roomFor = (id) => {
      const key = String(id);
      if (!byId.has(key)) {
        const known = (map.rooms || []).find((room) => String(room.id) === key);
        const room = {
          id: key,
          name: known ? known.name : `Room ${key}`,
          code: ROOM_CODE_BASE + rooms.length,
          cells: 0,
          sumX: 0,
          sumY: 0,
          bbox: {
    ...
    (map.rooms || []).forEach((room) => roomFor(room.id));

    const cells = new Array(w * h).fill(0);
    for (let y = 0; y < h; y += 1) {
      const rawRow = height - 1 - (y + crop.y);
      for (let x = 0; x < w; x += 1) {
        const cell = classOf(pixels[rawRow * width + x + crop.x] || 0);
        let code = 0;
        if (cell.kind === 'wall') code = 1;
        else if (cell.kind === 'floor') code = 2;
        else if (cell.kind === 'room') {
          const room = roomFor(cell.roomId);
          code = room.code;
          room.cells += 1;
          room.sumX += x;
          room.sumY += y;
          room.points.push(x, y);
          if (x < room.bbox.x0) room.bbox.x0 = x;
          if (y < room.bbox.y0) room.bbox.y0 = y;
          if (x > room.bbox.x1) room.bbox.x1 = x;
          if (y > room.bbox.y1) room.bbox.y1 = y;
        }
        cells[y * w + x] = code;
      }
    }
    const present = rooms.filter((room) => room.cells > 0);

    // Neighbouring rooms: room cells within two cells of each other (walls
    // and door frames sit between rooms).
    const neighbours = new Map(present.map((room) => [room.code, new Set()]));
    for (let y = 0; y < h; y += 1) {
      for (let x = 0; x < w; x += 1) {
        const a = cells[y * w + x];
        if (a < ROOM_CODE_BASE) continue;
        for (const [dx, dy] of [[1, 0], [2, 0], [0, 1], [0, 2]]) {
          if (x + dx >= w || y + dy >= h) continue;
          const b = cells[(y + dy) * w + x + dx];
          if (b >= ROOM_CODE_BASE && b !== a) {
    ...
    const toDisplay = (point) => (point ? {
      ...
      y: height - 1 - Math.round(point.y) - crop.y,
      theta: typeof point.theta === 'number' ? point.theta : null,
    } : null);
    const dock = toDisplay(map.dock);
    const robot = toDisplay(map.robot);

    // Greedy colouring in breadth-first order from the dock's room.
    const colorIndex = new Map();
    const dockCode = dock && dock.x >= 0 && dock.y >= 0 && dock.x < w && dock.y < h ? cells[dock.y * w + dock.x] : 0;
    const order = [...present].sort((p, q) => q.cells - p.cells);
    const start = order.find((room) => room.code === dockCode) || order[0];
    const queue = start ? [start.code] : [];
    const seen = new Set(queue);
    const visit = (code) => {
      const used = new Set([...neighbours.get(code)].map((n) => colorIndex.get(n)).filter((c) => c !== undefined));
      let c = 0;
      while (used.has(c)) c += 1;
      colorIndex.set(code, c % ROOM_COLOR_SETS);
    };
    for (let i = 0; i < queue.length || seen.size < present.length; i += 1) {
      if (i >= queue.length) {
        const next = order.find((room) => !seen.has(room.code));
        queue.push(next.code);
        seen.add(next.code);
      }
      visit(queue[i]);
      for (const n of neighbours.get(queue[i])) {
        if (!seen.has(n)) {
          seen.add(n);
          queue.push(n);
        }
    ...
    for (let i = 0; i < cells.length;) {
      let j = i + 1;
      while (j < cells.length && cells[j] === cells[i]) j += 1;
      runs.push(cells[i], j - i);
      i = j;
    }
    ...
    return {
      ...
      height: h,
      cells: runs,
      rooms: present.map((room) => ({
        id: room.id,
        name: room.name,
        code: room.code,
        colorIndex: colorIndex.get(room.code) || 0,
        cells: room.cells,
        bbox: room.bbox,
        label: labelPoint(room.points, room.sumX / room.cells, room.sumY / room.cells),
      })),
      dock,
      robot,
      updatedAt: map.updatedAt || Date.now(),
      meta: {
        mapId: map.meta && map.meta.mapId ? map.meta.mapId : null,
        resolution: map.meta && map.meta.resolution ? map.meta.resolution : 0,
        renderedAt: options.now || Date.now(),
        crop: {
          x: crop.x, y: crop.y, width: w, height: h, sourceWidth: width, sourceHeight: height,
        },
      },
    };
  }
  ...
  /**
   * A live position (robot pose or trail point, raw map coordinates) in the
   * display cells of the widget render: column = x - border.left, row from
   * the bottom = y - border.bottom, then flipped and cropped like the grid.
   */
  static liveToDisplay(point, border, renderMeta) {
    if (!point || !border || !renderMeta || !renderMeta.crop) return null;
    const { crop } = renderMeta;
    const round = (v) => Math.round(v * 10) / 10;
    return {
      x: round(point.x - border.left - crop.x),
      y: round(crop.sourceHeight - 1 - (point.y - border.bottom) - crop.y),
      theta: typeof point.theta === 'number' ? point.theta : null,
    };
  }

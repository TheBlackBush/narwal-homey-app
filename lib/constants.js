...
// A docked robot takes about 40 s to answer a new cloud link; before its first
// answer the cloud link gets this long instead.
const CLOUD_FIRST_ANSWER_TIMEOUT_MS = 180000;
// At most one map fetch per window, as the official app does.
const MAP_REFETCH_GUARD_MS = 10000;
// A map changed by live updates is re-emitted at most this often.
const MAP_UPDATE_EMIT_MS = 30000;
// Without a fresh map this session, a failed map request is retried after
// these delays (the last repeats).
const MAP_RETRY_DELAYS_MS = [60000, 120000, 300000];
// Over the cloud replies take 20 to 30 s. A command succeeds on its reply or
// on the robot's status showing the result, whichever comes first; it waits
// up to CLOUD_REPLY_TIMEOUT_MS for the reply and CLOUD_CONFIRM_TIMEOUT_MS in
// all for the status. The map has only its reply, so it waits longer.
const CLOUD_REPLY_TIMEOUT_MS = 12000;
const CLOUD_CONFIRM_TIMEOUT_MS = 32000;
const CLOUD_MAP_TIMEOUT_MS = 60000;
const POLL_INTERVAL_MS = 60000; // status polling fallback cadence
// The robot publishes for 60 s after an active_robot_publish request; renew
// at half of that, as the official app does.
const SUBSCRIPTION_RENEW_MS = 30000;
...
module.exports = {
  ...
  CLOUD_FIRST_ANSWER_TIMEOUT_MS,
  MAP_REFETCH_GUARD_MS,
  MAP_UPDATE_EMIT_MS,
  MAP_RETRY_DELAYS_MS,
  CLOUD_REPLY_TIMEOUT_MS,
  CLOUD_CONFIRM_TIMEOUT_MS,
  CLOUD_MAP_TIMEOUT_MS,
  POLL_INTERVAL_MS,
